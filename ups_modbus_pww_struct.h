#ifndef _UPS_MODBUS_POWERWARE_STRUCT_H_
#define _UPS_MODBUS_POWERWARE_STRUCT_H_

#include "ups_struct.h"


// Powerware state structure
typedef	struct tagPowerwareState
{
	struct PowerwareState
	{
		union PowerwareStateReg
		{
			struct _Bits
			{
...
//			unsigned short	_Data;
		...
		union PowerwareOutputTruePower
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareInputVoltagePhase
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareBypassVoltagePhase
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareOutputVoltagePhase
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareBypassFrequency
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareOutputFrequency
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareInputCurrentPhase
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
		...
		union PowerwareLoadCurrentPhase
		{
			unsigned short _Data;
			double realValue() {
				return ((double)_Data / 10.0);
...
//			RegState._Data = 0;
	...
	tagPowerwareState() : Updated()
	{
	}
} PowerwareState;


#endif // _UPS_MODBUS_POWERWARE_STRUCT_H_
